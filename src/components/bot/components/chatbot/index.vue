<template>
  <div id="chatbotwindow" ref="app" style="overflow-y:scroll;-webkit-overflow-scrolling : touch;">
    <!-- TopHead is the header with the information about the app -->
    <TopHead v-if="app" :app="app">
      <!-- Audio toggle (on the top right corner), used to toggle the audio output, default mode is defined in the settings -->
      <div
        :style="{ color: muted ? 'inherit' : 'white', 'padding-right': muted ? '15px' : '10px' }"
        class="audio-toggle"
        :title="
          muted
            ? (botconfig.i18n[lang()] && botconfig.i18n[lang()].unMuteTitle) ||
              botconfig.i18n[botconfig.app.fallback_lang].unMuteTitle
            : (botconfig.i18n[lang()] && botconfig.i18n[lang()].muteTitle) ||
              botconfig.i18n[botconfig.app.fallback_lang].muteTitle
        "
        :aria-label="
          muted
            ? (botconfig.i18n[lang()] && botconfig.i18n[lang()].unMuteTitle) ||
      ...
      >
        ...
        <i
          :class="{ 'fa-volume-up': !muted, 'fa-volume-off': muted }"
          class="fa material-icons"
          style="font-size:20px;"
          aria-hidden="true"
        />
      </div>
      <div class="close-toggle" @click="closeChat()">
        <i class="fa fa-sign-out material-icons fa" style="font-size:20px;" aria-hidden="true" />
      </div>
    </TopHead>
    <section class="chat-container">
      ...
      <section v-else aria-live="polite">
        <div v-for="m in messages" :key="m.responseId" class="message">
          <!-- My message -->
          <BubbleWrapper>
            <Bubble me :text="m.queryResult.queryText" />
          </BubbleWrapper>
          ...
          <section v-if="m.queryResult.webhookPayload && m.queryResult.webhookPayload.google">
            <div
              v-for="(component, id) in m.queryResult.webhookPayload.google.richResponse.items"
              :key="id"
              class="component"
            >
            ...
            <div
              v-for="(component, id) in m.queryResult.webhookPayload.google.systemIntent"
              :key="id"
              class="component"
            >
...
export default {
  ...
  data() {
    return {
      ...
      language: '',
      session: '',
      muted: this.botconfig.app.muted,
      loading: false,
      error: null,
      client: null,
    }
    ...
    suggestions() {
      if (this.messages.length > 0) {
        ...
        for (const component in last_message.queryResult.fulfillmentMessages) {
          if (last_message.queryResult.fulfillmentMessages[component].suggestions) {
          ...
          if (last_message.queryResult.fulfillmentMessages[component].linkOutSuggestion) {
            suggestions.link_suggestion = last_message.queryResult.fulfillmentMessages[component].linkOutSuggestion
          }
          if (last_message.queryResult.fulfillmentMessages[component].quickReplies) {
            suggestions.text_suggestions =
              last_message.queryResult.fulfillmentMessages[component].quickReplies.quickReplies
          }
        }
        ...
        if (last_message.queryResult.webhookPayload && last_message.queryResult.webhookPayload.google) {
          for (const component in last_message.queryResult.webhookPayload.google) {
            if (last_message.queryResult.webhookPayload.google[component].suggestions) {
              suggestions.text_suggestions = last_message.queryResult.webhookPayload.google[component].suggestions.map(
                suggestion => suggestion.title,
              )
            }
            if (last_message.queryResult.webhookPayload.google[component].linkOutSuggestion) {
              suggestions.link_suggestion = last_message.queryResult.webhookPayload.google[component].linkOutSuggestion
            }
          }
        }
      ...
      return {
        text_suggestions: this.botconfig.app.start_suggestions, // <- if no messages are present, return start_suggestions, from config.js to help user figure out what he can do with your application
      }
    },
  },
  watch: {
    ...
    loading() {
      setTimeout(() => {
        const app = document.querySelector('#chatbotwindow') // <- We need to scroll down #app, to prevent the whole page jumping to bottom, when using in iframe
        ...
        if (app.querySelector('.message')) {
          ...
          app.scrollTo({ top: message, behavior: 'smooth' })
        }
  ...
  methods: {
    ...
    send(q) {
      ...
      axios({
        method: 'get',
        url: 'http://localhost:5000/v1/chatbot/detect',
        params: request,
      })
        .then(response => {
          this.messages.push(response.data)
          this.handle(response.data) // <- trigger the handle function (explanation below)
          console.log(response.data) // <- (optional) log responses
        })
        .catch(error => {
          this.error = error.message
        })
      ...
      if (response.outputAudio) {
        ...
        if (response.queryResult.webhookPayload && response.queryResult.webhookPayload.google) {
          for (const component in response.queryResult.webhookPayload.google) {
            if (response.queryResult.webhookPayload.google[component].simpleResponse) {
              text = response.queryResult.webhookPayload.google[component].simpleResponse.textToSpeech
            }
          }
        }
        ...
        speech.voiceURI = this.botconfig.app.voice
